document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Render grid 8 brands ngẫu nhiên
  if (brandGrid) {
    ...
    const randomBrands = shuffle(brandsWithLogo).slice(0, 8);
  ...
  function renderBrands() {
    ...
    Object.keys(grouped).sort().forEach(k => {
      ...
      const brandHTML = grouped[k]
        .map(
          b => {
            const logoUrl = b.logo ? (b.logo.startsWith('http') ? b.logo : `/${b.logo}`) : '';
            return `
              <li class="brand-item">
                <a href="/thuong-hieu/${encodeURIComponent(b.name)}" style="text-decoration: none; color: inherit; display: flex; align-items: center; width: 100%;">
                  ...
                  ${
                    b.logo
                      ? `<div class="brand-logo"><img src="${logoUrl}" alt="${b.name} logo"></div>`
                      : ""
                  }
                </a>
              </li>
