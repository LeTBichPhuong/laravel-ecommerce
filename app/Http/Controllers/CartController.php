...
class CartController extends Controller
{
    ...
    {
        $userId = Auth::id();
        $cartItems = Cart::query()->where('user_id', '=', $userId)->get();
    ...
    {
        $userId = Auth::id();
        $cartItem = Cart::query()->where('user_id', '=', $userId)->where('id', '=', $id)->first();
    ...
    public function checkout(Request $request)
    {
        try {
            ...
            if ($paymentMethod === 'QR') {
                ...
                $storedTransferCode = strtoupper(session('qr_transfer_code') ?? '');

                if ($inputCode !== $storedCode && $inputCode !== $storedTransferCode && $inputCode !== 'DH' . $storedCode) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Mã giao dịch không hợp lệ! Vui lòng nhập đúng nội dung chuyển khoản: ' . ($storedTransferCode ?: 'DH' . $storedCode)
                    ], 400);
                }
                ...
                // Nếu mã đúng, lưu transaction_code (Đảm bảo luôn có DH ở đầu để khớp 100% nội dung CK)
                $transactionCode = (strpos($inputCode, 'DH') === 0) ? $inputCode : 'DH' . $inputCode;
                ...
                // Cho COD: Tự động tạo mã đơn hàng giống định dạng mã chuyển khoản (DH + random)
                $transactionCode = 'DH' . strtoupper(substr(md5(uniqid()), 0, 8));
            }
    ...
    private function parsePrice(string $priceString)
    {
